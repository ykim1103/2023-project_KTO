...
def code_sns_data_merge(sns_data,code_data,logger):
    try:
        code_real_df = pd.merge(sns_data,code_data,how='inner',on=['sido_nm','sgg_nm'])
        del code_real_df['sido_nm']
        del code_real_df['sgg_nm']         
...
if __name__ == "__main__":
    ...
    logger = loggers.make_logger(path,"valid_sgg")
    ...
    sns_data = data_loader.read_query(require_valid_start_date,require_valid_end_date,['sido_nm','sgg_nm','base_ymd'],logger)
    if len(sns_data) == 0:

...
def retrain(x_train_origin,y_train_origin,input_size,best_parameters,device,num_epoch,path,model_id,c,div,logger):
    ...
    torch.save(model,path+sub_path+model_id+f'/sns_predict_{c}.pt')
...
def test(x_test_origin,y_test_origin,path,model_id,c,device,div,logger):
    ...
    total_y_pred = []
    load_scaler = load(open(path+sub_path+model_id+f"/sns_data_scaler_{c}.pkl","rb"))
    model = torch.load(path+sub_path+model_id+f"/sns_predict_{c}.pt")
